...
from validators import max_length, Invalid, mandatory, numeric, in_set, latitude_longitude, set_equal, \
    no_trailing_whitespace, region_matches_treatment_code
...
class SampleValidator:
    ...
    def __init__(self):
        self.schema = {
            'UPRN': [mandatory(), max_length(12), numeric(), no_trailing_whitespace()],
            'ESTAB_UPRN': [mandatory(), max_length(12), numeric(), mandatory(), no_trailing_whitespace()],
            'ADDRESS_TYPE': [mandatory(), in_set({'HH', 'CE', 'SPG'}), no_trailing_whitespace()],
            'ESTAB_TYPE': [mandatory(), max_length(255), no_trailing_whitespace()],
            'ADDRESS_LEVEL': [mandatory(), in_set({'E', 'U'}), no_trailing_whitespace()],
            'ABP_CODE': [mandatory(), max_length(6), no_trailing_whitespace()],
            'ORGANISATION_NAME': [max_length(60), no_trailing_whitespace()],
            'ADDRESS_LINE1': [mandatory(), max_length(60), no_trailing_whitespace()],
            'ADDRESS_LINE2': [max_length(60), no_trailing_whitespace()],
            'ADDRESS_LINE3': [max_length(60), no_trailing_whitespace()],
            'TOWN_NAME': [mandatory(), max_length(30), no_trailing_whitespace()],
            'POSTCODE': [mandatory(), max_length(8), no_trailing_whitespace()],
            'LATITUDE': [mandatory(), latitude_longitude(max_scale=7, max_precision=9), no_trailing_whitespace()],
            'LONGITUDE': [mandatory(), latitude_longitude(max_scale=7, max_precision=10), no_trailing_whitespace()],
            'OA': [mandatory(), max_length(9), no_trailing_whitespace()],
            'LSOA': [mandatory(), max_length(9), no_trailing_whitespace()],
            'MSOA': [mandatory(), max_length(9), no_trailing_whitespace()],
            'LAD': [mandatory(), max_length(9), no_trailing_whitespace()],
            'REGION': [mandatory(), max_length(9), no_trailing_whitespace(), region_matches_treatment_code()],
            'HTC_WILLINGNESS': [mandatory(), in_set({'0', '1', '2', '3', '4', '5'}), no_trailing_whitespace()],
            'HTC_DIGITAL': [mandatory(), in_set({'0', '1', '2', '3', '4', '5'}), no_trailing_whitespace()],
            'FIELDCOORDINATOR_ID': [max_length(10), no_trailing_whitespace()],
            'FIELDOFFICER_ID': [max_length(13), no_trailing_whitespace()],
            'TREATMENT_CODE': [mandatory(), in_set(self.TREATMENT_CODES), no_trailing_whitespace()],
            'CE_EXPECTED_CAPACITY': [numeric(), max_length(4), no_trailing_whitespace()],
            'CE_SECURE': [mandatory(), in_set({'0', '1'}), no_trailing_whitespace()],
            'PRINT_BATCH': [numeric(), max_length(2), no_trailing_whitespace()]
        }
